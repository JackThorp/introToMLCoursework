...
% predicted emotion labels (1 to 6) for these examples.
    ...
    for i=1:no_emotions
        ...
        %classifications(:,i) = getClassesAndScores(T{i}, x2);
    end

    predictions = zeros(length(x2), 1);
    
    for x=1:length(x2)
        predictions(x) =  chooseEmotionUsingScores(classifications(x,:), scores(x,:));
        %predictions(x) =  chooseEmotionRandom(classifications(x,:));
    end
    
end

function [ classifications, scores ] = getClassesAndScores(tree, data)
%function [ classifications ] = getClassesAndScores(tree, data)
